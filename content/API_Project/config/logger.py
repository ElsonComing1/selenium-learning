...
def setup_logger():
    ...
    logger.add(
        str(log_file),
        level='DEBUG',
        format='{time:YYYY-MM-DD HH:mm:ss} | {level:<5} | {module:<15} | {function:<20} | {message}',
    ...
    logger.add(
        ...
        format='<green>{time:YYYY-MM-DD HH:mm:ss}</green> | <level>{level:<5}</level> | <cyan>{module:<15}</cyan> | {function:<20} | {message:<}',
        # 进行颜色设置
        colorize=True
    )
