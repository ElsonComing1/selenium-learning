...
class TestBaiduPom:
    ...
    def test_data_driven_search(self,driver,data):
        ...
        try:
            ...
            # for row in data:
                # print(row)
            case_id,case_name,keyword,expected_result,actual_result,status,remark=data
            baidu.search_content(keyword)
            actual_result=baidu.get_title()
            ...
            result_data.append([case_id,case_name,keyword,expected_result,actual_result,status,remark])
            # list接收多个对象而不是参数
...
if __name__=='__main__':
    ...
    # windows系统文件名不能包含：
    # print(PROJECT_PATH)
    report_name=os.path.join(PROJECT_PATH,'report',f'{TestBaiduPom.__name__}_{timestamp}_report.html')

    # print(f'afaefqwfwqe{report_name}')
    # print(PROJECT_PATH)
    pytest.main([__file__,'-v',f'--html={report_name}',\
                 '--self-contained-html'])